...
static omp_lock_t** create_lock_array(int N){
	omp_lock_t lock[N][N];
	int i, j;
    for (i = 1 ; i < N-1 ; i++)
		for (j = 1 ; j < N-1 ; j++){
...
int main (int argc, char * argv[]) {
	...
	for (t = 0 ; t < T ; t++) {
		#pragma omp parallel for collapse(2) reduction (+:current)
		for (i = 1 ; i < N-1 ; i++){
			for (j = 1 ; j < N-1 ; j++) {	
				//set all locks for surrounding neighbours
				omp_set_lock(&locks[i+1][j+1]); omp_set_lock(&locks[i+1][j]); omp_set_lock(&locks[i+1][j-1]);
				omp_set_lock(&locks[i][j-1]); 								  omp_set_lock(&locks[i][j+1]);
				omp_set_lock(&locks[i-1][j-1]); omp_set_lock(&locks[i-1][j]); omp_set_lock(&locks[i-1][j+1]);
	...
	printf("GameOfLifeOMP: Size %d Steps %d Time %lf\n", N, T, time);
	#ifdef OUTPUT
	system(FINALIZE);
	#endif
	return 0;
